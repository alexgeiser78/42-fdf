...
int	fdf_default(void *data)
{
	...
	dt->mapctr.width); //si la largeur est plus grande que la longueur on stocke dans l'argument scale la valeur de 
		printf("1 FDF_WIDTH'%d'/hyp(.width'%d', .width'%d') = %f\n",FDF_WIDTH, dt->mapctr.width, dt->mapctr.width, dt->scale);
		}
	else if (FDF_WIDTH <= FDF_HEIGHT)
	{
		dt->scale = FDF_WIDTH / hypotf(dt->mapctr.height, 
	...
	dt->mapctr.height)
	{
		dt->scale = FDF_HEIGHT / hypotf(dt->mapctr.width, 
	dt->mapctr.width);
		printf("3 FDF_HEIGHT'%d'/hyp(.width'%d', .width'%d') = %f\n",FDF_HEIGHT, dt->mapctr.width, dt->mapctr.width, dt->scale);//
	}
	else
	{
		dt->scale = FDF_HEIGHT / hypotf(dt->mapctr.height, 
...
//si on divise la hauteur ou largeur de la window par cette valeur on obtient l'echelle
/*
int	fdf_changecolor(t_mlx *dt)
{
	printf("-changecolor-\n");//
